...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--download", action="store_true", help="Download and unzip the Kaggle astronomy dataset before training.")
...
def download_kaggle_dataset(image_dir: Path, download_dir: Path) -> None:
...
def main() -> None:
    ...
    args = parse_args()
    if args.download:
        download_kaggle_dataset(args.image_dir, args.download_dir)

    paths = image_paths(args.image_dir, args.limit)
    if not paths and not args.no_download_if_missing and has_kaggle_auth():
        print(f"No local images found in {args.image_dir}. Downloading Kaggle dataset...")
        download_kaggle_dataset(args.image_dir, args.download_dir)
        paths = image_paths(args.image_dir, args.limit)
